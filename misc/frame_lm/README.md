...
**Version 0.3: Chat and Context Studio are included.** See `CHAT.md` for the chatbot, saved conversations, sources, and model-backend setup. Open `CONTEXT_STUDIO.md` for the
...
generating contexts. It uses only the Python standard library; no C++ compilation,
PyTorch, or model server is required for Context Studio.
...
**Delivery status:** the dependency-free context pipeline is runnable and tested.
...
Install Python 3.10 or later if needed. Extract this archive, open PowerShell in
the `frame_lm` folder, then run:

```powershell
python -m framelm index
python -m framelm ask "What is an abelian group?"
python -m framelm ask "How does context retrieval work?" --json
```

Use `py` instead of `python` if that is your Python launcher. These same commands
work in a Linux or macOS terminal. The default backend needs no pip packages and

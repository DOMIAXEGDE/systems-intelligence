...
class SQLiteRetriever:
    ...
    @observed('index.build', lambda self, *a, **kw: self.path)
    def build(self, directory: Path, chunk_words: int = 180, overlap: int = 30) -> int:
        ...
        try:
            ...
            outbox(db, 'index.committed', 'index', 'main', {'chunks':[dict(zip(('rowid','source','text'),r)) for r in db.execute('SELECT rowid,source,text FROM chunks')]})
            db.commit()
        except Exception:
            db.rollback()
            raise
    ...
    @observed('retrieval.search', lambda self, *a, **kw: self.path if self.path.exists() else None)
    def search(self, frame: Frame, top_k: int) -> list[Context]:
...
DEFAULTS = {
    ...
    "device": "cpu", "include_extractive_fallback": True, "stream_output": False,
}
...
def load_config(path: Path) -> dict:
    ...
    if type(cfg['stream_output']) is not bool: raise ValueError('stream_output must be boolean.')
...
class Pipeline:
    ...
    @observed('pipeline.run', lambda self, *a, **kw: self.config)
    def run(self, prompt: str, retrieval_query: str | None = None) -> dict:
        ...
        emit('pipeline.evidence', result, entities=['context:'+c.source.split(' / ')[0].removeprefix('context:') for c in contexts if c.source.startswith('context:')])
        ...
        candidates = []
        try:
            emit('backend.request', {'backend':cfg['backend'],'frame':asdict(frame),'contexts':[asdict(c) for c in contexts]}, phase='started')
            candidates = backend.generate(frame, contexts, cfg)
            emit('backend.response', {'candidates':[asdict(c) for c in candidates]}, phase='completed')
        except (RuntimeError, ValueError, OSError) as exc:
            emit('backend.failed', {'error':str(exc)}, phase='failed')
            if not cfg["include_extractive_fallback"]:
                raise
            result["warnings"].append(f"Requested backend failed: {exc}")
            emit('backend.fallback', {'error':str(exc)}, phase='failed')

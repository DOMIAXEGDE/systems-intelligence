...
from .controller.hooks import observed, scope
...
class StudioApp:
    def __init__(self, config):
        ...
        self.cancel = self.controller.cancel
        self.job = {"state": "idle", "processed": 0, "count": 0}
        self.worker = None
        self.controller.skills.start()
        self.controller.emit("controller.started", phase="completed")

    @observed("generator.start", lambda self, *a, **kw: self.config)
    def start_generation(self, recipe):
        ...
        def work():
            try:
                result = self.controller.generate(recipe,progress)
        ...
        self.worker = threading.Thread(target=lambda: context.run(work), daemon=True)
        self.worker.start()
...
def make_server(config, port=8765):
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            try:
                ...
                if url.path in {"/app.js", "/style.css", "/controller.js", "/controller.css"}:
                ...
                if url.path.startswith('/api/controller/download/'):
                    import re
                    name=url.path.rsplit('/',1)[-1]
                    if not re.fullmatch(r'[a-f0-9-]+\.(gif|zip)', name):
                        return self.respond({'error':'Invalid export filename'},400)
                    path=app.controller.directory/'exports'/name
                    return self.send_bytes(path.read_bytes(),'image/gif' if name.endswith('.gif') else 'application/zip')
                if url.path.startswith('/api/controller/'):
                    from .controller.web import get
                    return self.respond(get(app.controller,url.path[len('/api/controller/'):],query))
        ...
        def do_POST(self):
            ...
            try:
                ...
                if self.path.startswith('/api/controller/'):
                    ...
                    return self.respond(post(app.controller,self.path[len('/api/controller/'):],data))
                ...
                if self.path == "/api/preview":
                    return self.respond(app.controller.dispatch("generator.preview", {"recipe": data}, str(__import__("uuid").uuid4())))
                if self.path == "/api/generate":
                    return self.respond(app.start_generation(Recipe.from_dict(data)))
                if self.path == "/api/cancel":
                    app.cancel.set()
                    app.controller.cancel_generation()
    ...
    class ControllerServer(ThreadingHTTPServer):
        def server_close(self):
            app.cancel.set()
            if app.worker: app.worker.join(timeout=5)
            app.controller.close()
            super().server_close()

    server = ControllerServer(("127.0.0.1", port), Handler)
    server.app = app
    return server
